...
import { FriendCompareButton } from "@/app/components/FriendCompareButton";
...
export default async function FriendProfilePage({ params }: Props) {
  ...
  if (error === "Not authenticated") {
    const back = appHref(`/friend/${encodeURIComponent(rawId)}`);
    redirect(`${appHref("/login")}?redirect=${encodeURIComponent(back)}`);
  }
  if (error === "self") {
  ...
  const compareHref = `${appHref("/insights/strength-compare")}?with=${encodeURIComponent(data.subjectUserId)}`;
    ...
    <div className="flex min-h-[100dvh] flex-col bg-zinc-950 text-zinc-100">
      <div className="fixed inset-x-0 top-0 z-[210] bg-zinc-950 pt-[env(safe-area-inset-top,0px)]">
        <AppHeader
          title={
            <>
              <span className="min-w-0 truncate">{data.username}</span>
              {showCrownForLiftlyPro(data.liftly_pro) ? <RankCrownIcon size={20} title="Liftly Pro" /> : null}
            </>
          }
      ...
      <main className="mx-auto w-full max-w-xl flex-1 px-4 pb-[max(7rem,env(safe-area-inset-bottom,0px))] pt-[calc(3.5rem+env(safe-area-inset-top,0px))] sm:px-6">
        <div className="pt-4">
          ...
          {data.hasFriendDetailStats ? (
            <div className="mt-6">
              <AccountAchievementsCard
                sectionHeading={`${data.username}'s milestones`}
          ...
          {data.relationship === "friend" ? (
            <div className="mt-8">
              <FriendCompareButton href={compareHref} isFriend={data.relationship === "friend"} />
            </div>
          ) : null}
        </div>
      </main>
